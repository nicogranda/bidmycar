  ...
  const handleSubmit = (e) => {
    ...
    fetch('http://localhost:8888/bidmycar/backend/api/products/create.php', {
      ...
      .then(result => {
        ...
        // Asegurarnos de que 'vehicle_id' se pasa correctamente desde 'inserted_id'
        const vehicle_id = result.inserted_id; // Asignamos 'inserted_id' a 'vehicle_id'
    
        if (result.status === 'success' && vehicle_id) {
          //alert('Formulario enviado correctamente');
          // Regresamos a la versión original de la navegación
          navigate('/historico', { state: { vehicle_id } });
        } else {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          <div className="checkbox-group">
            <label>
              <div className="checkbox-with-gap">
                <input
                  type="checkbox"
                  name="featured"
                ...
                />
                <span>Quiero destacar este coche</span>
              </div>
            </label>
          </div>
        </div>
